...
<?php
   ...
   $sql =<<<EOF
      SELECT * from Task WHERE Flag LIKE 'P';
EOF;
			...
			<?php while($row = $ret->fetchArray(SQLITE3_ASSOC) ){?>
				<div class="row directory-listing">
                                ...
                                <span class="name col-md-3 col-sm-4 hidden-xs text-right">
                                    <?php echo '<a href="moveTo.php?id='.urlencode($row['ID']).'&flag='.urlencode("T").'">To Do</a>
												<span>|</span>
												<a href="moveTo.php?id='.urlencode($row['ID']).'&flag='.urlencode("D").'">Done</a>
												<span>|</span>
